...
function createChart(svg, data) {
  ...
  /*
  ------------------------
  CODE: define the y and x scales
  ------------------------
  */

  // Define y scale with new domain
  ...
  var yTicks = currentSVG.append("g")
    ...
    .call(d3.axisLeft(y)
      ...
      .tickFormat("") // Remove labels for now
    );

  // Y axis with labels only at 250 and 700
  ...
  yAxis.selectAll("text")
    ...
    .attr("dx", "1.4em"); // This moves the labels to the right by "1em". Adjust as necessary.
  ...
  /*
------------------------
CODE: preprocess your data
------------------------
  */
  // Main data transformation
  const newDataReady = preprocessData(dataReady);
  const sortedData = filterAndSortData(newDataReady[0].values);
  const errorLookup = computeErrorLookup(newDataReady[0].values);

  // Append line segments to SVG based on the sorted data
  function appendLineSegmentsToSVG(sortedData, svg, newDataReady) {
    const wave = newDataReady[0].values.find(v => v.time === 'Wave').value;

    for (let i = 0; i < sortedData.length - 1; i++) {
      if (['Wave', 'Country', 'Benchmark'].includes(sortedData[i].time)) continue;

      const segmentData = [sortedData[i], sortedData[i + 1]];

      // Skip segments where either point has NaN coordinates
      if (isNaN(x(+segmentData[0].time)) || isNaN(y(+segmentData[0].value)) || isNaN(x(+segmentData[1].time)) || isNaN(y(+segmentData[1].value))) {
        continue;
      }

      const segment = d3.line()
        .defined(d => !isNaN(d.value) && d.value != null && d.value !== '0')
        .x(d => x(+d.time))
        .y(d => y(+d.value));
  ...
  appendLineSegmentsToSVG(sortedData, currentSVG, newDataReady);

  // Data attributes
  dataReady[0].values.forEach(v => currentSVG.attr(`data-${v.time}`, v.value));


  /*
------------------------
CODE: draw the d3 points
------------------------
  */
  // Function to handle data based on whether d is available
  function handleData(d) {
    return d ? d.values : [];
  }

  // Draw the error bars and rectangles based on the data
  function drawErrorBarsAndRectangles(d) {
    let errorBarLength = errorLookup[d.time.replace('_error', '')] || 0;
    errorBarLength = isNaN(errorBarLength) ? 0 : errorBarLength;

    let yPos = y(d.value);
    let xPos = x(d.time);
    let yErrorBottom = y(d.value - errorBarLength);
    let diff = (yErrorBottom - yPos)
    let yErrorTop = yPos - diff;

    let tBarWidth = 5;  // Width of the horizontal bar (adjust this value as needed)

    if (!isNaN(xPos) && !isNaN(yPos) && !isNaN(errorBarLength)) {

      // Main vertical Error bars
      d3.select(this)
        .append("line")
        .attr("class", "errorBar")
        .attr("x1", xPos)
        .attr("y1", function(d) { // Use t
          //console.log(yPos - (errorBarLength * .75))
          console.log(errorBarLength)
          console.log('//////')
          console.log(yErrorTop)
          console.log(yPos)
          console.log(yErrorBottom)
          console.log(d)
          console.log('//////')
          console.log(yPos - (errorBarLength * .75))
          console.log(yPos)
          console.log(yPos + (errorBarLength * .75))
          console.log('//////////////////////////////////////////')
          //return yPos - (errorBarLength * .75)
          return yErrorTop
        })
        .attr("x2", xPos)
        .attr("y2", function(d) { // Use t
          //return  yPos + (errorBarLength * .75)
          return yErrorBottom
        })
        .attr("stroke", "black")
      ...
      // Top horizontal bar (cap)
      d3.select(this)
        .append("line")
        .attr("class", "errorBarCap")
        .attr("x1", xPos - tBarWidth / 2)
        .attr("y1", yErrorTop)
        .attr("x2", xPos + tBarWidth / 2)
        .attr("y2", yErrorTop)
        .attr("stroke", "black")
        .attr("transform", "translate(35, 0)")
        .attr("stroke-width", 1);

      // Bottom horizontal bar (cap)
      d3.select(this)
        .append("line")
        .attr("class", "errorBarCap")
        .attr("x1", xPos - tBarWidth / 2)
        .attr("y1", yErrorBottom)
        .attr("x2", xPos + tBarWidth / 2)
        .attr("y2", yErrorBottom)
        .attr("stroke", "black")
        .attr("transform", "translate(35, 0)")
        .attr("stroke-width", 1);
      ...
      d3.select(this)
        .append("rect")
        .attr("class", "myDots")
        .attr("x", xPos - 3).attr("y", y(d.value) - 1)
        .attr("width", 6).attr("height", 2)
        .attr("stroke-width", 0)
        .attr("stroke", "none")
        .attr("transform", "translate(35, 0)")
        .attr("data-value", d.value).attr("data-time", d.time)
        //if d.time = 2021, make it pink
        .attr("fill", d.time === 2021 ? (wave === '2' ? '#E9CBCC' : '#9D0505') : '#9D0505')
        .style("opacity", d.value == 0 || isNaN(d.value) ? 0 : 1);
    }
  }

  // Main function to draw points and error bars
  currentSVG.selectAll("myDots")
    .data(dataReady).enter()
    .append('g').style("fill", '#9D0505')
    .selectAll("myPoints")
    .data(handleData).enter()
    .each(drawErrorBarsAndRectangles);

  // Main function to draw text labels for each point
  currentSVG.selectAll("myText")
    .data(dataReady).enter()
    .append('g')
    .selectAll("myPoints")
    .data(handleData).enter()
    .append("text")
    .attr("x", d => !isNaN(x(d.time)) ? x(d.time) + 2 : 0)  // default to 0 if NaN
    .attr("y", d => !isNaN(y(d.value)) ? y(d.value) - 8 : 0) // default to 0 if NaN
    .text(d => d.value)
  ...
  /*
  ------------------------
  CODE: create the tooltip and hover functions
  ------------------------
  */
  ...
  const mouseover = function(d) {
    ...
    const currentScore = parseFloat(d3.select(this).attr("data-2001")).toFixed(2);
    tooltipHandler.displayTooltip(`PIRLS ${currentYear} average achievement: <b>${currentScore}</b>`, window.event.clientX, window.event.clientY + 100);
  }
  ...
  const mousemove = function(d) {
    ...
    const roundedYear = tooltipHandler.roundToNearestYear(currentDate);
    tooltipHandler.displayTooltip(tooltipHandler.generateTooltipContent(this, roundedYear), window.event.clientX, window.event.clientY);
  }
...
document.getElementById('downloadButton1').addEventListener('click', function() {
  ...
  htmlToPngDownload(svgElement, 'PIRLS2021_Exhibit 2.1_2.2.png');
...
async function getCountriesData() {
  ...
  function appendToBenchmarkDiv(chart, benchmark0Div, benchmark1Div) {
    // Access the underlying DOM element from the D3 selection
  ...
  function addBenchmarkTitleIfNecessary(benchmark1Div) {
    ...
    if (!document.querySelector('.benchmarkTitle')) {
      // If no element with the class 'benchmarkTitle' exists on the page
...
*/
  d3.csv("https://html-css-js.jadesign.repl.co/data/averageAchievement.csv", function(error, data) {
    ...
    /*
    ------------------------
    METHOD: you have two containers, for benchmark and nonbenchmark particapants.
    ------------------------
    */
    ...
    //go through every country in your data, extract the data and attach it to the div, then append and draw an svg 
    for (var country in transformedData) {
      ...
      const years = [2001, 2006, 2011, 2016, 2021]; // Update this array if more years are added
      years.forEach((year, index) => {
        chart.attr(`data-${year}`, function() {
          return (data[index].Average != null && data[index].Average != undefined) ? data[index].Average : 0;
        });
      });

      chart.attr("data-benchmark", data[7].Average || 0)
        .attr("data-country", data[5].Average || 0)
    ...
    */
    ...
    d3.selectAll(".dropdownMenu").on("change", function() {
      ...
      updateBenchmarks(sortedCharts, benchmark0Div, benchmark1Div); // <- Pass the two divs here
    });
    ...
    */
    ...
    function updateChartVisibility() {
      ...
      d3.selectAll('.chart').each(function() {
        ...
        // If the country of the chart is in the currentClickedButtons, show it, otherwise hide it
      ...
      // If currentClickedButtons is empty, show all charts
    ...
    function attachSelectedCountryEventListeners() {
      ...
      selectedResultsButtons.on('click', function() {
        ...
        // Remove the clicked country from currentClickedButtons
        let index = currentClickedButtons.indexOf(currentCountryButtonName);
        if (index > -1) {
          currentClickedButtons.splice(index, 1);
        }

        // Update currentlySelectedCountriesDataset based on currentClickedButtons
        const currentlySelectedCountriesDataset = currentClickedButtons.map(country => transformedData[country]);

        // Remove the clicked country button from the UI
        currentCountryButton.remove();

        // Restore the state of the corresponding result button in the search dropdown
        //d3.select(`.resultButton#${currentCountryButtonName}`).classed('resultButtonSelected', false);
        ...
        // Re-attach event listeners since the DOM might have changed
    ...
    searchInput.addEventListener('input', event => {
      ...
      searchResultsButtons.on('click', function() {
        //check how many elements with the class selectedCountry are on the page
        if (d3.selectAll(".selectedCountry").size() == 0) {
          // Code to execute if there are no elements with class 'chart'
          currentClickedButtons = []
        }
        let button = d3.select(this);
        button.classed("resultButtonSelected", true);
        const clickedCountry = d3.select(this).text();
        if (!currentClickedButtons.includes(clickedCountry)) {
          currentClickedButtons.push(clickedCountry);
        }
        updateChartVisibility();
        showClicked(currentClickedButtons, data);
        attachSelectedCountryEventListeners();  // This function will handle attaching event listeners to the selected countries.


        /* 
        This is where you call the displayInformation code to update the count of hiddden countries 
        */
        d3.select('.dropdownMenu').dispatch('change');
        /* 
      This isthe end of that code
      */
      });
    });
  })
